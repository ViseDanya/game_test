...
void LocalGameScreen::handleEvents(const SDL_Event& event)
{
  Screen::handleEvents(event);
  if (event.type == SDL_EVENT_MOUSE_WHEEL && manualCamera)
  {
    camera.zoom *= (event.wheel.y > 0) ? 1.1f : 0.9f;
  }
}
...
void LocalGameScreen::render(SDL_Renderer* renderer)
{
  Renderer bruh_renderer(renderer);

  updateAnimators(registry);
  updateTrampolineAnimations(registry);
  updateFakeAnimations(registry);
  updateAnimations(registry);
  renderColoredEntities(registry, bruh_renderer, camera);
  renderSprites(registry, bruh_renderer, camera);

  if(debugColliders)
  {
    renderDebugColliders(registry, bruh_renderer, camera);
  }
...
void LocalGameScreen::showImGui()
{
